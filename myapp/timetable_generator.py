...
all_slots1 = [("Afternoon", slot) for slot in afternoon_slot] + [("Morning", slot) for slot in morning_slots]
...
def generate_timetable(year, department, timetable, existing_timetables):
    ...
    labs_list = labs_filtered.to_dict('records') * 2  # If slicing is True, use this line
    # labs_list = labs_filtered.to_dict('records')  # If slicing is False, use this line
    i = 0
    ...
    for day in days:
        ...
        while lab_no < len(labs_list):
            if lab_no == len(labs_list):  # stopping Empty labs_list
                break
            if not lab_room_slicing:
                break  # no slicing

            time_of_day, three_periods = all_slots[i % len(all_slots)]
            if faculty_collision:
                time_of_day, three_periods = all_slots[(i + 1) % len(all_slots1)]
            ...
            if not faculty_collision and room_available:
                ...
                # Print reasons for not placing the lab
                if faculty_collision:
                    print(f"Faculty collision: Lab {lab_name} with faculty {faculty_name} could not be placed.")
                if not room_available:
                    print(f"Room availability issue: Lab {lab_name} could not be placed in room {room_no}.")
                lab_no += 1
        ...
        # Check for remaining labs that were not placed
        if labs_list:
            print(f"Remaining labs that were not placed for {year} - {department}:")
            for lab_item in labs_list:
                print(f"Lab Name: {lab_item['lab_name']}, Faculty: {lab_item['faculty']}, Room No: {lab_item['room_no']}")
