    ...
    for day in days:
        while len(labs_list) > 0:
            ...
            while lab_no < len(labs_list):
                ...
                slot_list = ['Free'] if pd.isna(slot_values) else ast.literal_eval(slot_values)
                ...
                if not faculty_collision and room_available:
                    if lab_slicing == 1:
                        ...
                        paired_lab_found = False
                        for next_lab_no in range(lab_no + 1, len(labs_list)):
                            paired_lab_item = labs_list[next_lab_no]
                            if (paired_lab_item['slice'] == 1 and 
                                    paired_lab_item['faculty'] != faculty_name and 
                                    paired_lab_item['room_no'] == room_no):
                                paired_lab_name = paired_lab_item['lab_name']
                                paired_lab_faculty = paired_lab_item['faculty']

                                # Place both labs
                                for period in three_periods:
                                    if timetable.at[day, periods[period]] == "Free":
                                        timetable.at[day, periods[period]] = [(lab_name, faculty_name), (paired_lab_name, paired_lab_faculty)]
                                    else:
                                        timetable.at[day, periods[period]].extend([(lab_name, faculty_name), (paired_lab_name, paired_lab_faculty)])

                                    update_faculty_timetable(faculty_name, day, periods[period], f"{lab_name} ({year}, {department})")
                                    update_faculty_timetable(paired_lab_faculty, day, periods[period], f"{paired_lab_name} ({year}, {department})")

                                slot_list[idx] = (year, department, lab_name, faculty_name)
                                lab_rooms.loc[lab_rooms['Day'] == day, room_col] = str(slot_list)
                                lab_rooms.to_csv(lab_rooms_path, index=False)

                                # Remove placed labs from the list
                                labs_list.pop(next_lab_no)
                                labs_list.pop(lab_no)
                                paired_lab_found = True
                                break

                        if paired_lab_found:
                            break
                        else:
                            # If no pair is found, backtrack and try a different slot
                            lab_no += 1
                            continue

                    else:
                        # Place the lab if it's not a slicing lab
                        for period in three_periods:
                            if timetable.at[day, periods[period]] == "Free":
