...
def generate_timetable(year, department, timetable, existing_timetables):
    ...
    # Create lists based on the importance value and slice
    labs_list_slice_1 = []
    labs_list_slice_0 = []
    for _, lab in labs_filtered.iterrows():
        if lab['slice'] == 1:
            labs_list_slice_1.extend([lab.to_dict()] * lab['importance'])
        else:
            labs_list_slice_0.extend([lab.to_dict()] * lab['importance'])
    
    # Function to place labs from the provided list
    def place_labs(labs_list):
        i = 0
        for day in days:
    ...
    # First place labs with slice value 1
    place_labs(labs_list_slice_1)
    
    # Then place labs with slice value 0
    place_labs(labs_list_slice_0)
